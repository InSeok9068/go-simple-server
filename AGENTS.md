...
2. 코드 파일을 수정한 작업은 완료 전에 반드시 `./task.sh check`를 실행하고 통과해야 합니다.
3. 코드 파일 수정 작업에서 `./task.sh check` 실패 시 원인 수정 후 재실행합니다.
...
- 이 저장소는 하나의 코드베이스에서 여러 서비스를 함께 운영하는 모노리포입니다.
- 공통 코드는 루트 `internal/`, `pkg/`에 두고, 서비스 전용 코드는 `projects/{project}/` 아래에 둡니다.
- 의존 방향은 공통 레이어의 서비스 역참조 금지, 서비스 간 직접 import 금지를 기본 원칙으로 합니다.
...
3. 코드 파일을 수정한 경우 마지막에 반드시 Git Bash에서 아래를 실행합니다.

```bash
./task.sh check
```

4. 코드 파일 수정 작업에서 실패하면 수정 후 3번을 반복합니다.
5. 결과 보고 시 아래를 함께 남깁니다.

- 변경 파일
- 생성 명령 실행 여부
- 코드 파일 수정 시 `./task.sh check` 통과 여부
...
4. 코드 파일 수정 작업에서 검증 명령 생략 후 완료 선언 금지
