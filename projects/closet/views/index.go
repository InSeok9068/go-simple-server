		...
		Div(Class("padding stack gap-sm"),
			...
			Div(x.Show("$store.auth.isAuthed"),
				Form(Class("stack gap-sm"),
					...
					Attr("hx-on::after-request", "if (event.detail.successful) { this.reset(); document.getElementById('upload-dialog').close(); showInfo('새 옷을 추가했어요. 임베딩을 준비 중입니다.'); }"),
					h.On("htmx:response-error", "showError(event.detail.xhr.responseText || '업로드에 실패했어요. 다시 시도해 주세요.');"),
